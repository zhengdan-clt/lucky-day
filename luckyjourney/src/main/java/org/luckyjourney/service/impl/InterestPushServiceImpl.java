...
public class InterestPushServiceImpl implements InterestPushService {
    ...
    public Collection<Long> listVideoIdByTypeId(Long typeId) {
        // 随机推送10个
        final List<Video> list = redisTemplate.opsForSet().randomMembers(RedisConstant.SYSTEM_TYPE_STOCK + typeId, 12);
        final HashSet<Long> result = new HashSet<>();
        //如果list本身为空，直接查数据库,然后存入Redis中
        if(list == null || list.isEmpty()) {
            List<Video> videos = videoMapper.selectList(new LambdaQueryWrapper<Video>().eq(Video::getTypeId, typeId));
            if (videos != null && !videos.isEmpty()) {
                // 将数据库结果添加到 Redis 集合中
                for (Video video : videos) {
                    redisTemplate.opsForSet().add(RedisConstant.SYSTEM_TYPE_STOCK + typeId, video);
                }
            }
            for (Video video : videos) {
                if (video!=null){
                    result.add(Long.parseLong(video.getId().toString()));
                }
            }
        }else{
            //list不为空的情况
