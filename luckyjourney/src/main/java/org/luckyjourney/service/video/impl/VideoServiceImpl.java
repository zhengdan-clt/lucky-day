...
public class VideoServiceImpl extends ServiceImpl<VideoMapper, Video> implements VideoService {
    ...
    public boolean startVideo(Long videoId) {
        ...
        final UserModel userModel = UserModel.buildUserModel(labels, videoId, result ? 1.0 : -1.0);
        interestPushService.updateUserModel(userModel);

        //添加一次浏览记录，覆盖掉之前的
        historyVideo(videoId, UserHolder.get());
    ...
    public void historyVideo(Long videoId, Long userId) {
        ...
        if (o == null) {
            ...
            redisCacheUtil.zadd(RedisConstant.USER_HISTORY_VIDEO + userId, new Date().getTime(), videoId.toString(), RedisConstant.HISTORY_TIME);
            //增加浏览量
            updateHistory(video, 1L);
        }
        // 如果刷到过视频，也需要再添加一次浏览记录，因为有可能会有点赞，收藏等操作，得更新Redis。
        else{
            //移除旧的记录
            redisTemplate.opsForZSet().remove(RedisConstant.USER_HISTORY_VIDEO + userId,videoId.toString());
            //获取新的
            final Video video = getById(videoId);
            //添加新的记录
            redisCacheUtil.zadd(RedisConstant.USER_HISTORY_VIDEO + userId, new Date().getTime(), videoId.toString(), RedisConstant.HISTORY_TIME);
    ...
    public LinkedHashMap<String, List<Video>> getHistory(BasePage basePage) {
        ...
        for (ZSetOperations.TypedTuple<Object> typedTuple : typedTuples) {
            ...
            final Video video = videoMapper.selectById(Long.valueOf(typedTuple.getValue().toString()));
