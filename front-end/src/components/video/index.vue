<template>
  <v-layout v-if="currentVideo" full-height>
    ...
    gap: 12px;position: absolute; background-color: transparent; right: 25px; bottom: 25px;z-index: 99999;">
          ...
          <v-badge color="red" :content="currentVideo.shareCount" location="bottom">
...
const copyUrl = () => {
  shareBtn();
  const isCopied = strUtils.copyContent(location.host + "/#/?play=" + currentVideo.value.id);
  
  snackbar.value = {
    text: isCopied ? "视频地址复制成功" : "视频地址复制失败",
    show: true
  };

  // 确保 shareCount 已初始化，然后自增
  if (isCopied) {
    // 如果 shareCount 未定义，则初始化为 0
    if (currentVideo.value.shareCount === undefined) {
      currentVideo.value.shareCount = 0;
    }
    currentVideo.value.shareCount++;
  }
};
